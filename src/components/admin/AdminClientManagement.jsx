...
import {
  Plus, Trash2, Loader2, Check, Users, ChevronDown, ChevronUp,
  Pencil, Monitor, ShieldAlert, Ticket, CreditCard, X, Search,
  Building2, Server, Clock, CheckCircle2, AlertTriangle
} from "lucide-react";
...
function TeamDetailPanel({ team, users, allGroups, tickets, services, onSave, onDelete, onClose }) {
  ...
  const teamTickets = tickets.filter(t => members.some(m => m.email === t.client_email));
  const teamServices = services.filter(s => members.some(m => m.email === s.client_email));

  const openTickets = teamTickets.filter(t => t.status === "open" || t.status === "in_progress").length;
  const activeServices = teamServices.filter(s => s.status === "active").length;
  ...
  return (
    <div className="fixed inset-0 z-50 flex">
      ...
      <div className="relative ml-auto w-full max-w-2xl h-full bg-card border-l border-border/60 flex flex-col shadow-2xl overflow-hidden">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="text-xs text-muted-foreground mt-0.5 flex gap-3">
                ...
                {monthlySpend > 0 && <span>£{monthlySpend}/mo</span>}
              </div>
            </div>
          </div>
        ...
        {/* Tabs */}
        <div className="flex gap-1 px-6 pt-4 pb-0 flex-shrink-0 border-b border-border/30">
          {TABS.map(t => (
        ...
        <div className="flex-1 overflow-y-auto p-6">
                ...
                <>
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div className="p-4 rounded-xl border border-border/40 bg-background/40">
                      ...
                      {team.action1_group_id && <div className="text-xs text-muted-foreground mt-1 font-mono truncate">{team.action1_group_id}</div>}
                    </div>
                  </div>
          ...
          {tab === "members" && (
            <div className="flex flex-col gap-3">
              <div className="text-xs text-muted-foreground mb-1">Check/uncheck to add or remove members from this team. Changes save immediately.</div>
              {clientUsers.length === 0 ? (
                <p className="text-sm text-muted-foreground py-6 text-center">No client users found.</p>
              ) : (
                clientUsers.map(u => {
                  const isMember = team.member_emails?.includes(u.email);
                  return (
                    <div key={u.id} className={`flex items-center gap-3 p-3.5 rounded-xl border transition-all cursor-pointer ${isMember ? "border-primary/30 bg-primary/5" : "border-border/30 bg-card/30 hover:border-border/60"}`}
                      onClick={async () => {
          ...
          {tab === "services" && (
            <div className="flex flex-col gap-4">
              {teamServices.length === 0 ? (
                <div className="text-center py-10 text-muted-foreground text-sm">No services for this team.</div>
              ) : (
                <>
                  <div className="p-4 rounded-xl border border-border/30 bg-card/30 flex items-center justify-between">
                    <span className="text-sm text-muted-foreground">Total monthly spend</span>
                    <span className="text-lg font-extrabold">£{monthlySpend.toLocaleString()}</span>
                  </div>
                  {teamServices.map(s => (
                    <div key={s.id} className="p-4 rounded-xl border border-border/30 bg-card/30 flex items-center gap-3">
                      <div className="w-8 h-8 rounded-lg bg-primary/10 flex items-center justify-center flex-shrink-0">
                        <Server className="w-3.5 h-3.5 text-primary" />
                      </div>
                      <div className="flex-1 min-w-0">
                        <div className="text-sm font-medium">{s.service_name}</div>
                        <div className="text-xs text-muted-foreground">{s.client_email} · £{s.monthly_cost}/mo</div>
                      </div>
...
export default function AdminClientManagement({ users, tickets, services, onRefresh }) {
  ...
  const handleSave = () => {
    loadTeams();
    onRefresh();
    // Re-fetch selected team to reflect changes
    if (selectedTeam) {
  ...
  // Find the full latest team object when detail panel is open
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h1 className="text-xl font-extrabold flex items-center gap-2">
            <Building2 className="w-5 h-5 text-primary" /> Client Management
          </h1>
          <p className="text-sm text-muted-foreground mt-0.5">Teams group your clients with their services, tickets, ABR keys and endpoints.</p>
        </div>
      ...
      {showForm && (
        <form onSubmit={handleCreate} className="p-5 rounded-2xl border border-primary/25 bg-primary/5 flex flex-col gap-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            <div className="flex flex-col gap-1 sm:col-span-2">
              <label className="text-xs text-muted-foreground">Members (optional — can add later)</label>
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex flex-wrap gap-3 text-xs text-muted-foreground">
                      ...
                      {spend > 0 && <span className="flex items-center gap-1"><CreditCard className="w-3 h-3" />£{spend}/mo</span>}
